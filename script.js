// ── Firebase Config ───────────────────────────────────────────────────────────
// Loaded via CDN in index.html — firebase is a global variable
...
// Firestore document where all meetings are stored
// Collection: "minuteslog" / Document: "meetings"
// Stored as { list: [...] } — same pattern as krisync app
...
async function loadFromFirestore() {
    try {
        ...
        if (snap.exists) {
            ...
            // First run — empty, save blank doc
            meetings = [];
...
// ── Real-time Listener ────────────────────────────────────────────────────────
// Any change from another device instantly updates this UI
...
function bindEvents() {
    ...
    document.addEventListener('keydown', e => {
        ...
        if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') {
            if (document.getElementById('modalOverlay').style.display !== 'none') saveModal();
        }
    });
}
...
window.copyMeeting = function(id) {
    const m = meetings.find(x => x.id === id);
    if (!m) return;
    let txt = `${m.title}\n${fmtDateFull(m.date)}${m.time ? ' at ' + fmtTime(m.time) : ''}\n`;
